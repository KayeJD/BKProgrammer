...
with dpg.file_dialog(directory_selector=False, show=False, callback=create_csv_chart_data, tag="csv_sender", width=700,
                     height=400):
            ...
            with dpg.tab_bar():
                with dpg.tab(label="MANUAL mode"):
                    ...
                    dpg.add_button(label='Update Plot', width=-1,
                                   callback=lambda: create_manual_chart_data(dpg.get_value("shape")))

                # CSV table
                with dpg.tab(label="CSV mode"):
                    dpg.add_button(label='Select a .csv file', width=-1, callback=lambda:dpg.show_item("csv_sender"))
                    dpg.add_separator()
